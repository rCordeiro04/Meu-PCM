...
st.markdown(
    ...
    <style>
        ...
        /* DESIGN EXECUTIVO COMPACTO E ELEGANTE PARA OS CARDS KPI */
...
if tela == "Painel Correias":
    ...
    # 4 Cartões KPI com os novos rótulos padronizados (Tipos Totais, Tipos Nova, Tipos Meia Vida, Tipos para Troca)
    k1, k2, k3, k4 = st.columns(4)

    with k1:
        st.markdown("<div class='kpi-btn-box kpi-total'>", unsafe_allow_html=True)
        if st.button(f'<span class="kpi-titulo">Tipos Totais</span><span class="kpi-valor-container"><span class="kpi-val-txt">{len(lista_correias_todas)}</span><span>📦</span></span>', key="btn_kpi_total", use_container_width=True):
    ...
    with k2:
        ...
        if st.button(f'<span class="kpi-titulo">Tipos Nova</span><span class="kpi-valor-container"><span class="kpi-val-txt">{len(lista_correias_novas)}</span><span>🟢</span></span>', key="btn_kpi_novas", use_container_width=True):
    ...
    with k3:
        ...
        if st.button(f'<span class="kpi-titulo">Tipos Meia Vida</span><span class="kpi-valor-container"><span class="kpi-val-txt">{len(lista_correias_meia)}</span><span>🟡</span></span>', key="btn_kpi_meia", use_container_width=True):
    ...
    with k4:
        ...
        if st.button(f'<span class="kpi-titulo">Tipos para Troca</span><span class="kpi-valor-container"><span class="kpi-val-txt">{len(lista_correias_criticas)}</span><span>🔴</span></span>', key="btn_kpi_crit", use_container_width=True):
    ...
    if st.session_state.card_selecionado_kpi is not None:
        ...
        if sel == "TODAS":
            ...
            titulo_faixa = "📦 Tipos Totais de Correias"
        elif sel == "NOVAS":
            lista_alvo = lista_correias_novas
            titulo_faixa = "🟢 Tipos Nova (≤ 1 ano)"
        elif sel == "MEIA":
            lista_alvo = lista_correias_meia
            titulo_faixa = "🟡 Tipos Meia Vida (1 a 1,5 anos)"
        else:
            lista_alvo = lista_correias_criticas
            titulo_faixa = "🔴 Tipos para Troca (> 1,5 anos)"
